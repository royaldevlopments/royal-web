...
export default function Sidebar({ open, setOpen }) {
  ...
  const NavItem = ({ item }) => (
    <NavLink to={item.path} className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-sidebar-hover hover:text-foreground'}`}>
  ...
  return (
    <aside className={`${open ? 'w-60' : 'w-0'} transition-all duration-300 bg-sidebar border-r border-border overflow-hidden flex flex-col`}>
      <div className="p-4 flex items-center justify-between border-b border-border shrink-0">
        ...
        <button onClick={() => setOpen(false)} className="p-1 rounded-lg hover:bg-sidebar-hover transition-colors">
      ...
      <div className="border-t border-border p-2 space-y-1">
        ...
        <Link to="/" className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm text-primary hover:bg-primary/10 transition-colors">
